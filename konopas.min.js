...
item_not_found:function(e){return"Program id <b>"+e.ID+"</b> not found!"},
...
star_hint:function(){return'To "star" a program item, click on the square next to it. Your selections will be remembered, and shown in this view. You currently don\'t have any program items selected, so this list is empty.'},
...
KonOpas.prototype.set_view=function(){var e=window.location.hash.substr(1,4),t=_el("tabs");if(this.program&&this.program.list.length)switch(t.style.display="block",this.people&&this.people.list.length?t.classList.remove("no-people"):(t.classList.add("no-people"),"part"==e&&(e="")),e){case"part":this.people.show();break;case"star":this.stars.show();break;case"info":this.info.show();break;default:this.program.show(),e="prog"}else e="info",t.style.display="none",this.info.show(),this.server&&this.server.error("Program loading failed!");for(var n=0;n<this.views.length;++n)document.body.classList[e==this.views[n]?"add":"remove"](this.views[n]);_el("load_disable")&&(_el("load_disable").style.display="none")},
